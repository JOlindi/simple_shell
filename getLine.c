	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			remove_comments(*buf);
			build_history_list(infor, *buf, info->histcount++);
...
ssize_t read_buf(info_t *infor, char *buf, size_t *i)
{
	ssize_t r = 0;

	if (*i)
		return (0);
	r = read(infor->readfd, buf, READ_BUF_SIZE);
	if (r >= 0)
		*i = r;
	return (r);
}
...
int _getline(info_t *infor, char **ptr, size_t *length)
{
